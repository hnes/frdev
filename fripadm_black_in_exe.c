...
int main(int argc, char *argv[])
{
	...
	if(type==FR_IP_IOCTL_TYPE_FIND_BYSTRINGS) {
		if(argc==3)  {
			ret=double_hash_find_bystrings(fd,argv[2],strlen(argv[2]));
			if(ret==1) {
				printf("%s is in the double_hash_table\n",argv[2]);
				ret=0;
				goto end;
			}
		}
		...
		if(argc==3)  {
			ret=double_hash_mirror_insert_bystrings(fd,argv[2],strlen(argv[2]));
			goto end;
		}
		else {
			fprintf(stderr,"%s usage: ... \n",argv[0]);
		...
		if(argc==3)  {
			ret=double_hash_mirror_delete_bystrings(fd,argv[2],strlen(argv[2]));
			goto end;
		}
		else {
			fprintf(stderr,"%s usage: ... \n",argv[0]);
		...
		if(argc==2)  {
			ret=double_hash_mirror_delete_all(fd);
			goto end;
		}
		else {
			fprintf(stderr,"%s usage: ... \n",argv[0]);
		...
		if(argc==2)  {
			ret=double_hash_switch_mirror_update(fd);
			goto end;
		}
		else {
			fprintf(stderr,"%s usage: ... \n",argv[0]);
		...
		if(argc==4)  {
			ret=double_hash_rebuild(fd,(unsigned int)atoi(argv[2]),(unsigned int)atoi(argv[3]));
			goto end;
		}
		else {
			fprintf(stderr,"%s usage: ... \n",argv[0]);
